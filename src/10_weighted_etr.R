...
aggregate_etrs <- function(results, imports_gtap) {

  # --- Overall ETR by date ---
  overall <- results %>%
    group_by(date, label) %>%
    summarise(
      etr = sum(total_rate * imports) / sum(imports),
      total_imports_b = sum(imports) / 1e9,
      .groups = 'drop'
    ) %>%
  ...
  overall %>%
    ...
    select(date, label, etr_pct, total_imports_b) %>%
    print()
  ...
  by_partner <- results %>%
    ...
    summarise(
      etr = sum(total_rate * imports) / sum(imports),
      import_share = sum(imports),
      .groups = 'drop'
    ) %>%
    group_by(date) %>%
    mutate(import_share = import_share / sum(import_share)) %>%
    ungroup() %>%
    mutate(label = factor(label, levels = POLICY_DATES$label))
  ...
  by_authority <- results %>%
    group_by(date, label) %>%
    summarise(
      etr_total = sum(total_rate * imports) / sum(imports),
      etr_232 = sum(net_232 * imports) / sum(imports),
  ...
  # --- By GTAP sector (top 10 by import value) ---
  by_gtap <- results %>%
    inner_join(
...
if (sys.nframe() == 0) {
  ...
  results <- compute_weighted_etrs(data)
  etrs <- aggregate_etrs(results, data$imports_gtap)
